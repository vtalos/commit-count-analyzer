...
git_log=$(git log --pretty=format:%aD --since="2000-01-01" --until="2004-12-31" |
...
git_log=$(git log --pretty=format:%aD --since="2005-01-01" --until="2009-12-31" |
cut -d, -f1 | sort | uniq -c)
echo "$git_log" | while read -r row; do
...
git_log=$(git log --pretty=format:%aD --since="2010-01-01" --until="2014-12-31" |
cut -d, -f1 | sort | uniq -c)
echo "$git_log" | while read -r row; do
num_commits=$(echo "$row" | awk '{print $1}')
echo "$num_commits">> ../column.csv
...
git_log=$(git log --pretty=format:%aD --since="2015-01-01" --until="2019-12-31" |
cut -d, -f1 | sort | uniq -c)
echo "$git_log" | while read -r row; do
num_commits=$(echo "$row" | awk '{print $1}')
echo "$num_commits">> ../column.csv
...
git_log=$(git log --pretty=format:%aD --since="2020-01-01" --until="2023-12-31" |
cut -d, -f1 | sort | uniq -c)
echo "$git_log" | while read -r row; do
num_commits=$(echo "$row" | awk '{print $1}')
echo "$num_commits">> ../column.csv
