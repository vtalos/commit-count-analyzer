...
while IFS= read -r project_name; do
    ...
    commits_at_specific_hour=$(git log --after="$target_year-01-01" --before="$target_year-12-31" --pretty=format:%cd --all | grep -c '.*23:.*')
    commits_at_other_hours=$(git log --after="$target_year-01-01" --before="$target_year-12-31" --pretty=format:%cd --all | grep -vc '.*23:.*')
    # Accumulate counts
